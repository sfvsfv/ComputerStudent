...
void init(AMGraph &AM){
    ...
    strcpy(AM.view[i].data.name,"桃花林");
    ...
    strcpy(AM.view[i].data.name,"绮园");
    strcpy(AM.view[i].data.intro,"拥有一个小山丘,及众多果树.");
    i++;
    AM.view[i].num=i;
    strcpy(AM.view[i].data.code,"7");
    strcpy(AM.view[i].data.name,"交通大楼");
    strcpy(AM.view[i].data.intro,"是电子电气工程学院的学生进行实验的地方，以及老师的办公室.");
    i++;
    AM.view[i].num=i;
    strcpy(AM.view[i].data.code,"8");
    strcpy(AM.view[i].data.name,"湖边");
    strcpy(AM.view[i].data.intro,"从图书馆前的桥下穿过,旁边有许许多多的柳树，河里有很多鱼儿.");
    i++;
    AM.view[i].num=i;
    strcpy(AM.view[i].data.code,"9");
    strcpy(AM.view[i].data.name,"宿舍");
    strcpy(AM.view[i].data.intro,"是同学们睡觉休息的地方.");
    i++;
    AM.view[i].num=i;
    strcpy(AM.view[i].data.code,"10");
    strcpy(AM.view[i].data.name,"湿地公园");
